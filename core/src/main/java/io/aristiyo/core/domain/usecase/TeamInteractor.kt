...
class TeamInteractor
    @Inject
    constructor(
        private val teamRepository: ITeamsRepository,
    ) : TeamUseCase {
        override fun getAllTeams(): Flow<ResultStatus<List<Team>>> = teamRepository.getAllTeams()

        override fun getFavouriteTeam(): Flow<List<Team>> = teamRepository.getFavouriteTeam()
